...
function movingBall() {
    ...
    let interval = setInterval(() => {
        ...
        if (balls.top <= rod1.bottom && (balls.left >= rod1.left && balls.right <= rod1.right)) {
            yDirection = -yDirection;
            // console.log(yDirection)
        }

        if (balls.bottom >= rod2.top && (balls.left >= rod2.left && balls.right <= rod2.right)) {
            yDirection = -yDirection;
        }
...
function movingRods() {
    ...
    document.addEventListener('keydown', (e) => {
        // console.log(e)
        // let code = e.code
        if (e.key == 'ArrowLeft' && rod[0].offsetLeft >= 190 || (e.key == 'a' && rod[0].offsetLeft >= 190)) {
            rod[0].style.left = (rod[0].offsetLeft - u) + 'px';
            rod[1].style.left = (rod[0].offsetLeft - v) + 'px';
        } else if (e.key == 'ArrowRight' && (rod[0].offsetLeft + rod[0].offsetWidth) <= (window.innerWidth + 155) || e.key == 'd' && (rod[0].offsetLeft + rod[0].offsetWidth) <= (window.innerWidth + 155)) {
